...
class TDapp(wx.Panel):
    def __init__(self,parent):
        ...
        self.ip = wx.TextCtrl(self, -1, "172.16.121.34", pos=(30,40),size=(120, -1))
    ...
    def  sectimeget(self, event):#listbox2事件--获取数据库号--获取顶级轮次--次级轮次--写入listbox3
        try:
            ...
            if time_td =='timeout':
                ...
                msg1 = socket_data.stringtolist(time_td) #str转list
                msg_num = msg1.index("$")
                msg_arrw1 = msg1[0:msg_num]
                msg_arrw2 = msg1[msg_num+1:]
                self.treereset(msg_arrw2,msg_arrw1)
        except:
    ...
    def  thirdtimeget(self, event):#次级轮次
        #try:
            ...
            #print self.thirdmssql
            #f =self.listBox3.GetSelection()#获取当前选择的顶级轮次号
            #self.thirdmssql = self.listBox3.GetString(f)#通过号获取当前次级轮次名          
            if self.thirdmssql == "none_td" or cmp(self.thirdmssql_encode, "轮次子项") == 0:
                self.doyouwant = 1
            else:
                self.doyouwant = 0
                msg = ["thirdtime_td"]
                msg.append(self.thirdmssql)
                strmsg = socket_data.listtostring(msg) #list转str
                time_td =  socket_data.sendmsg(self.ips, self.port, strmsg) #发送&&接收
                if time_td == "thok":
                    pass
    ...
    def  excel_charts_out(self,pathdst_cvs_excel,excel_arrw):
        ...
        socket_data.excelChart(self.excel_arrw, pathdst_cvs_excel)
...
class  TDsec(wx.Panel): #功能2
    ...
    def excelout2(self, event):        
        ...
        if time_td == 'fail':
            ...
            wx.FutureCall(1000,self.showsusscebox)  
    #def  communion(self,event):
        #ipp = r"\\172.16.0.25\share"
        #os.system("explorer.exe %s" % ipp)      
    #================功能二=checklist======================
...
class DemoFrame(wx.Frame):
    def __init__(self):
        wx.Frame.__init__(self, None, -1, 'TD系统数据统计分析工具1.0',pos=(500, 50),size=(600, 500),style=wx.DEFAULT_FRAME_STYLE ^(wx.RESIZE_BORDER | wx.MAXIMIZE_BOX))
