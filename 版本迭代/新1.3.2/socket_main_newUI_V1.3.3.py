...
class TDapp(wx.Panel):
    ...
    def progress_delete(self):
        '''停止线程函数'''
        while self.threads:
            threada = self.threads[0]
            threada.stop()
            self.threads.remove(threada)
            print "close"    
...
class aboutUI(wx.Frame):
    def __init__(self):
        ...
        self.stat7=wx.StaticText(self.panel,-1,"  版本: 1.3.2.20151211",(80,80))
...
class DemoFrame(wx.Frame):
    def __init__(self):
        wx.Frame.__init__(self, None, -1, 'TD系统数据统计分析工具1.3',pos=(500, 50),size=(600, 500),style=wx.DEFAULT_FRAME_STYLE ^(wx.RESIZE_BORDER | wx.MAXIMIZE_BOX))
...
class jindutiao_Frame(wx.Frame):  
    '''进度条框架'''
    def __init__(self):  
        wx.Frame.__init__(self, None, 10, '数据获取中...', size = (500, 200),style =wx.SYSTEM_MENU|wx.MINIMIZE_BOX|wx.CLOSE_BOX|wx.CAPTION)  
...
class thrd(threading.Thread):
    ...
    def  run(self):
        """运行脚本调用主循环中函数"""
        wx.CallAfter(self.window.progress)
    #----------------------------------------------------------------------
    def  stop(self):
        """停止脚本调用线程中函数"""
        dio = jindutiao_Frame()
