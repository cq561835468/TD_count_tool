    ...
    def  timestatistics(self,event):#统计按照时间执行的STEP
        try:
            if self.doyouwant == 1:
                ...
                time_td =  socket_data.sendmsg(self.ips, self.port, strmsgs) #发送&&接收
                if time_td == 'fail':
                    wx.FutureCall(1000,self.showfailbox) 
                else:
                    time_tds = time_td.replace(" ","_")                                 #过滤文件名中空格
...
class  TDthird(wx.Panel): #功能2
    def __init__(self,parent):
        ...
        self.button1 = wx.Button(self, -1, "导出excel", pos=(230, 370))
        self.Bind(wx.EVT_PAINT,  self.autologin)  
        self.Bind(wx.EVT_LISTBOX,  self.timeget, self.listBox1)
        self.Bind(wx.EVT_LISTBOX,  self.sectimeget, self.listBox2)
        self.Bind(wx.EVT_BUTTON, self.out_feature3, self.button1)
        #--------------------树控件     
        self.Bind(wx.EVT_TREE_SEL_CHANGED, self.thirdtimeget, self.tree)     
        self.doyouwant = 0 #变量判断符        
    def  autologin(self,event):
...
class aboutUI(wx.Frame):
    def __init__(self):
        ...
        self.stat7=wx.StaticText(self.panel,-1,"  版本: 1.6.2.20160601",(80,80))
...
class DemoFrame(wx.Frame):
    def __init__(self):
        wx.Frame.__init__(self, None, -1, 'TD系统数据统计分析工具1.6.2',pos=(500, 50),size=(600, 500),style=wx.DEFAULT_FRAME_STYLE ^(wx.RESIZE_BORDER | wx.MAXIMIZE_BOX))
